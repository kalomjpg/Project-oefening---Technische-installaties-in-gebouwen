...
Zie Tabel \ref{tab:dimensionering_kanalen} voor de resultaten en figuur \ref{fig:ventilatietracé} voor het ventilatietracé. 

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{ventilatietracéprintoverleaf.pdf}
    \caption{ventilatietracé}
    \label{fig:ventilatietracé}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l c c c c c}
        \toprule
        \rowcolor{gray!20}
        \textbf{Kamer} & \textbf{$V_{\text{ex}}$} & \textbf{$\theta_i$} & \textbf{$V_{\text{ex}} \cdot \theta_i$} & \textbf{Effectiviteit} & \textbf{$\theta_{m,\text{ex}}$} \\
        \rowcolor{gray!20}
        ~ & [\unit{\cubic\meter\per\hour}] & [$^\circ$C] & [\unit{\cubic\meter\per\hour} $\cdot$ $^\circ$C] & \textbf{warmtewisselaar [-]} & [$^\circ$C] \\
        \midrule
        Open keuken       & 116,80 & 20 & 2336,00 & 0,83 & 19,23 \\
        Badkamer 1        & 75,00  & 24 & 1800,00 & 0,84 & ---   \\
        Badkamer 2        & 75,00  & 24 & 1800,00 & 0,85 & ---   \\
        Technische ruimte & 50,00  & 16 & 800,00  & 0,86 & ---   \\
        Hal 1             & 100,00 & 16 & 1600,00 & 0,87 & ---   \\
        Hal 2             & 100,00 & 16 & 1600,00 & 0,88 & ---   \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\subsubsection{Inregeling van bijkring A}

Voor de berekening van de hydraulische inregeling is bijkring A geselecteerd. Het theoretische drukverlies van deze bijkring ($\Delta p_A$) is de som van de respectievelijke lineaire en plaatselijke drukverliezen, wat resulteert in 1712,84 Pa. 
...
\begin{equation}
    \Delta p_{inregel} = \Delta p_{C} - \Delta p_{A} = 2304,86 \text{ Pa} - 1943,47 \text{ Pa} = 361,39 \text{ Pa}
\end{equation}
...
Om het hydraulisch evenwicht in de praktijk te realiseren, wordt het berekende extra drukverschil over de bijkring weggewerkt door middel van een specifieke voorinstelling op de thermostatische radiatorkraan. Voor bijkring A bedraagt dit in te regelen drukverschil ($\Delta p_{inregel}$) $361,39 \text{ Pa}$, oftewel afgerond $0,36 \text{ kPa}$. 

Het benodigde massadebiet ($q_{m,rad}$) voor het verwarmingslichaam in deze kring (de kleine slaapkamer) is eerder vastgesteld op \qty{53,67}{\kilo\gram\per\hour}, wat in de praktijk gelijkgesteld mag worden aan een volumedebiet van circa \qty{54}{\liter\per\hour}. 

Aan de hand van de karakteristieke nomogrammen voor radiatorkranen (conform de ontwerprichtlijnen uit het WTCB-Rapport 14) kan de correcte kraanstand worden bepaald. Het snijpunt van het ontwerpdebiet (\qty{54}{\liter\per\hour})en het te creëren extra drukverlies (\qty{0,36}{\kilo\pascal}) resulteert in het voorliggende ontwerp in \textbf{stand 5} (zie figuur\ref{fig:drukverliesthermostaatkraan}). Deze voorinstelling garandeert dat de betreffende bijkring de exacte ontwerpweerstand levert, waardoor het waterdebiet correct verdeeld wordt en oververhitting van deze kortere kring wordt voorkomen.
