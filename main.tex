...
\begin{equation}
    ...
    \label{eq:placeholder_label}
\end{equation}
De effectiviteit van de warmtewisselaar is terug te vinden in de technische fiche van het ventilatiesysteem (zie bijlage).
...
\toprule
\rowcolor{gray!20}
\textbf{Ruimte} & \textbf{$A_t$ [$m^2$]} & \textbf{$v_{50}$} & \textbf{$H_{inf}$ [W/K]} & \textbf{$\theta_i$ [$^\circ$C]} & \textbf{$\theta_x$ [$^\circ$C]} & \textbf{$\Phi_{V,inf}$ [W]} \\ \midrule
...
Initieel rekenen we met de maximaal toegelaten watersnelheid om de minimale buisdiameter te bepakle
\begin{equation}
    {v_{max} = 0.5}
\end{equation} 
...
We kunnen ook echter voor het lineair drukverlies per lengte eenheid gaan kijken naar de toebehorende waarde uit tabel A7 van het WTCB-rapport. Het resultaat hiervan is te vinden in tabel \ref{tab:lineaire_drukverliezen}.
...
\begin{table}[!ht]
    ...
    \begin{tabularx}{\textwidth}{l l X c c}
        ...
        \midrule
        --- & --- & Terugslagklep & 6,0 & \textbf{22,0} \\
        ~   & ~   & Pomp          & 10,0 & ~ \\
        ~   & ~   & Terugslagklep & 6,0 & ~ \\
        \midrule
        A & A1 & Ronde bocht & 0,5 & \textbf{15,0} \\
        ~ & ~  & Ronde bocht & 0,5 & ~ \\
        ~ & ~  & Collector (vertrek) & 0,5 & ~ \\
        ~ & ~  & Radiator & 2,5 & ~ \\
        ~ & ~  & Antiterugslagklep & 4,0 & ~ \\
        ~ & ~  & Afsluitkraan & 6,0 & ~ \\
        ~ & ~  & Collector (retour) & 1,0 & ~ \\
        \midrule
        B & B1 & Ronde bocht & 0,5 & \textbf{15,0} \\
        ~ & ~  & Ronde bocht & 0,5 & ~ \\
        ~ & ~  & Collector (vertrek) & 0,5 & ~ \\
        ~ & ~  & Radiator & 2,5 & ~ \\
        ~ & ~  & Antiterugslagklep & 4,0 & ~ \\
        ...
        ~ & ~  & Collector (retour) & 1,0 & ~ \\
        \midrule
        C & C1 & Ronde bocht & 0,5 & \textbf{15,0} \\
        ~ & ~  & Ronde bocht & 0,5 & ~ \\
        ~ & ~  & Collector (vertrek) & 0,5 & ~ \\
        ~ & ~  & Radiator & 2,5 & ~ \\
        ~ & ~  & Antiterugslagklep & 4,0 & ~ \\
...
\begin{table}[!ht]
    ...
    \begin{tabular}{cc}
        \toprule
        \rowcolor{gray!20}
        \textbf{Kring} & \textbf{Plaatselijk drukverlies} \\
        \rowcolor{gray!20}
        \textbf & [\unit{\pascal}] \\
        \midrule
        A & 1943,474651 \\
...
Voor de berekening van de hydraulische inregeling is bijkring A geselecteerd. Het theoretische drukverlies van deze bijkring ($\Delta p_A$) is de som van de respectievelijke lineaire en plaatselijke drukverliezen, wat resulteert in 1943,47 Pa. 
...
\section{Technische Fiche ventilatiesysteem}
\label{app:TFventiunit}
